...
OUTPUT_LOG_FILE="${SCRIPT_DIR}/classification_analysis_${TIMESTAMP}.log"

if command -v tmux &> /dev/null
then
    echo "tmux is available. Running the classification analysis in a tmux session..."
    tmux new-session -d -s "$TMUX_SESSION_NAME" "bash $SCRIPT_DIR/run_all_classification_analysis.sh 2>&1 | tee $OUTPUT_LOG_FILE; read -n 1 -s -r -p 'Press any key to exit...'"
