...
class Clustering:
    def __init__(self, centroids, tabdata, k_min, k_max, k_step, cluster_on='samples',
        ...
        self.labels = {}
        self.n_init_local = n_init_local
        self.n_init_global = n_init_global

    @staticmethod
    def _select_cluster_data(tabdata, cluster_on):
        data = tabdata.scaled if tabdata.scaled is not None else tabdata.data
        if cluster_on == 'samples':
            return data.T
        return data


    @classmethod
    def init_local_clustering(cls, tabdata, k_min, k_max, k_step, seed_value, cluster_on='samples',
                              n_init_local=50, n_init_global=50) -> 'Clustering':
        """
    ...
    def aggregate_centroids(self, centroid_dict):
        for k in centroid_dict:
            ...
            clud = clu.KMeans(n_clusters=k, n_init=self.n_init_global, random_state=self.rs)
    ...
    def simplified_silhouette_scores(self,  label, centroids):
        ...
        for i in range(self.data.shape[0]):
            ...
            for c in range(centroids.shape[0]):
                # compute distance
                cd = np.linalg.norm(self.data[i, :] - centroids[c, :])
    ...
    def to_tsv(self, centroids_file, clustering_file, silhouette_file, delim='\t'):
        for k in self.k_list:
            ...
            if self.cluster_on == "samples":
                names = self.tabdata.columns
            else:
                names = self.tabdata.rows
            pd.DataFrame(self.labels[k], index=names).to_csv(clu, sep=delim)
            pd.DataFrame(self.silhouette_scores[k], index=names).to_csv(silh, sep=delim)
