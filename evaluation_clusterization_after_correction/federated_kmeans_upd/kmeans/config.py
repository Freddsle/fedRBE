...
from kmeans.params import INPUT_DIR, OUTPUT_DIR
...
class FCConfig:
    def __init__(self):
        ...
        self.n_init_local = 50
        self.n_init_global = 50
        self.max_global_iter = 300

    def parse_configuration(self):
        print('[API] /setup parsing parameter file ')
        regex = re.compile(r'^config_kmeans.*\.(?:yaml|yml)$')          # Config file pattern
        ...
        if op.exists(config_file):
            ...
            with open(config_file, 'r') as file:
                ...
                try:
                    self.seed_value = parameter_list['algorithm']['seed']
                    if self.seed_value == "random":
                        self.seed_value = np.random.randint(0, 1000000)
                    else:
                        self.seed_value = int(self.seed_value)
                except KeyError:
                    print('Seed not specified, defaulting to 11')
                    self.seed_value = 11

                try:
                    cluster_on = parameter_list['algorithm']['cluster_on']
                    cluster_on = str(cluster_on).strip().lower()
                    if cluster_on in ['samples', 'sample', 'columns', 'column', 'cols', 'col']:
                        self.cluster_on = 'samples'
                    elif cluster_on in ['features', 'feature', 'rows', 'row']:
                        self.cluster_on = 'features'
                    else:
                        raise ValueError
                except KeyError:
                    print('cluster_on not specified, defaulting to samples')
                    self.cluster_on = 'samples'
                except ValueError:
                    print('cluster_on invalid, defaulting to samples')
                    self.cluster_on = 'samples'

                print('Seed value: ' + str(self.seed_value))

                try:
                    self.n_init_local = parameter_list['algorithm']['n_init_local']
                except KeyError:
                    print('n_init_local not specified, defaulting to 50')
                    self.n_init_local = 50

                try:
                    self.n_init_global = parameter_list['algorithm']['n_init_global']
                except KeyError:
                    print('n_init_global not specified, defaulting to 50')
                    self.n_init_global = 50

                try:
                    self.max_global_iter = parameter_list['algorithm']['max_global_iter']
                except KeyError:
                    print('max_global_iter not specified, defaulting to 300')
                    self.max_global_iter = 300
