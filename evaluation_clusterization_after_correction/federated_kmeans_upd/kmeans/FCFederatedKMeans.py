...
from kmeans.TabData import TabData
import pandas as pd
import traceback
import numpy as np
import copy
from kmeans.params import INPUT_DIR, OUTPUT_DIR
from kmeans.COParams import COParams
import time
from kmeans.Clustering import Clustering
from kmeans.serialize import *
import shutil
...
class FCFederatedKMeans:
    ...
    def copy_configuration(self, config):
        ...
        self.seed_value = config.seed_value
        self.cluster_on = config.cluster_on
        self.n_init_local = config.n_init_local
        self.n_init_global = config.n_init_global
        self.max_global_iter = config.max_global_iter
    ...
    def init_kmeans(self):
        self.clustering = Clustering.init_local_clustering(
            self.tabdata, self.k_min, self.k_max, self.k_step, self.seed_value,
            cluster_on=self.cluster_on,
            n_init_local=self.n_init_local,
            n_init_global=self.n_init_global,
        )
    ...
    def aggregate_centroids(self, incoming):
        ...
        converged = self.iteration_counter >= self.max_global_iter
        print(incoming)
        ...
        # Aggregate all clients' centroids into global centroids
        self.clustering.aggregate_centroids(local_centroids)
