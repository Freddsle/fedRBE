...
5. `evaluation_clusterization_after_correction/real_datasets/00_build_kmeans_matrices.ipynb` — stacks the three per-modality corrected matrices into the joint k-means-ready matrices `after/all_modalities_{before,corrected}_kmeans_matrix.tsv` and, if `FedApp_corrected_data.tsv` is present for all modalities, also `after/all_modalities_fedsim_kmeans_matrix.tsv`. Writes `after/all_modalities_metadata.tsv`.
