...
For Metabolomics, the Figshare matrix is already `log2(expr + 1)`. Exact zeros are treated as non-detections **only on a temporary copy used to compute per-client detection rates**; the correction matrix itself keeps the original zeros. Each client independently keeps metabolites detected in at least 50% of its samples. Metabolites kept by at least three clients are retained, and the surviving per-client feature sets are merged by union: a retained metabolite absent from one client is represented by zeros for that client's samples (no extra imputation). The L04 fold-in (`T_L4_01` added to `client_03_L05_L04` to balance modality sample counts) is independent from `INCLUDE_CLIENT_04` and is preserved in metadata via `lab="L04"`.
