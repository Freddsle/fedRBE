...
@dataclass
class Experiment():
    clients: List[str]  # List of client folders, relative to the data folder!
    app_image_name: str # The name of the app image to be used,  
                        # expects the latest tag to be used! 
    config_files: List[dict] # List of all config files to be used as dicts, 
                            # order is the name as clients
    ...
    generic_dir: Union[str, None] = None    # The generic folder, relative to the data folder!
    ...
    result_folder: Union[str, None] = None   # The result folder, None for default (data/tests) (relative to the data folder!)
                                             # This is where the FINAL results are saved, intermedieate results are saved in 
                                             # data_folder/tests due to featurecloud limitations
...
def start_test(experiment: Experiment, data_dir: str, retry: int = 0, ) -> int:
    ...
    """
    print(f"Starting a test with the following configuration: {experiment}")
    ### Check prerequisites
